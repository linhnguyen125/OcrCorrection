...
class BasicDataset(Dataset):
    ...
    def remove_random_accent(self, text, ratio=0.15):
        """
        Xóa ngẫu nhiên dấu từ các từ trong câu\n
        param: ratio - tỉ lệ xóa dấu
        """
        words = text.split()
    ...
    def remove_random_space(self, text):
        """
        Xóa ngẫu nhiên khoảng trắng trong câu
        """
        words = text.split()
        n_words = len(words)
    ...
    def _random_replace(self, text, match_chars):
        """
        Hàm thay thế ngẫu nhiên một ký tự trong chuỗi
        """
    ...
    def replace_accent_chars(self, text, ratio=0.15):
        """
        Hàm thay thế ngẫu nhiên một ký tự có dấu thành ký tự không dấu
        """
        words = text.split()
        mask = np.random.random(size=len(words)) < ratio
